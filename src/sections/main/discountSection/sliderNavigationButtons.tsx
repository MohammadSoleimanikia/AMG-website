...
export default function SliderNavButtons({
  ...
  return (
    <div dir="ltr" className="flex h-fit items-center gap-3 rounded-2xl px-1 py-2.5">
      <ButtonBase
        type="button"
        onClick={handlePrevSlide}
        className={clsx(
          type === 'primary'
            ? 'bg-custom-4 hover:bg-background-paper'
            : 'bg-background-default',
          'flex size-10 items-center justify-center rounded-full text-text-primary transition-colors duration-300',
        )}
      >
        <FaAngleLeft
          className={clsx(type === 'primary' && 'text-primary-main', 'size-4')}
        />
      </ButtonBase>
      <ButtonBase
        type="button"
